...
- (void)viewDidLoad
{
    ...
    self.navigationBar.mainLabel.text = NSLocalizedStringFromTable(@"pick city", @"SearchHotel", @"");
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    NSArray *citiesInSection = [self.citiesSections objectAtIndex:section];
    if (section == 0 && row == 0) {
        cell.textLabel.text = NSLocalizedStringFromTable(@"current position", @"SearchHotel", nil);
        return cell;
    } 
...
{
    [self.cityListDelegate selectedCity:nil];
}
