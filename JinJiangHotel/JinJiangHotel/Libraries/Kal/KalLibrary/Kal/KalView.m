...
static const CGFloat kMonthLabelHeight = 23.f;

@implementation KalView

@synthesize delegate, tableView;
//@synthesize dateCheckDelegate;
...
- (id)initWithFrame:(CGRect)frame delegate:(id<KalViewDelegate>)theDelegate logic:(KalLogic *)theLogic
{
  if ((self = [super initWithFrame:frame])) {
    ...
    [self addSubview:headerView];  
      ...
      const unsigned int hh = frame.size.height - kHeaderHeight;
    UIView *contentView = [[[UIView alloc] initWithFrame:CGRectMake(0.f, kHeaderHeight, frame.size.width, hh)] autorelease];
...
//{
//    dateCheckDelegate=theDelegate;
//    [gridView initDateCheckDelegate:dateCheckDelegate];
//}
...
- (void)addSubviewsToHeaderView:(UIView *)headerView
{
  ...
  headerTitleLabel.textColor = [UIColor colorWithRed:67.0/255.0 green:67.0/255.0 blue:67.0/255.0 alpha:1.0];
//  headerTitleLabel.shadowColor = [UIColor whiteColor];
  ...
  for (CGFloat xOffset = 0.f; xOffset < headerView.width; xOffset += 46.f, i = (i+1)%7) {
    CGRect weekdayFrame = CGRectMake(xOffset, 36.f, 46.f, 23);
    ...
    weekdayLabel.text = NSLocalizedStringFromTable([weekdayNames objectAtIndex:i], @"SearchHotel", @"");
...
{
    [gridView getDateRangeMin:min Max:max];
}
...
{
    [gridView initWithCheckInDate:dateIn CheckOutDate:dateOut];
}
- (void)setCheckInDate:(NSDate *)dateIn CheckOutDate:(NSDate *)dateOut
{
    [gridView setCheckInDate:dateIn CheckOutDate:dateOut];
}
