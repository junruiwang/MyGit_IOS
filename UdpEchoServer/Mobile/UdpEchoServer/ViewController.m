...
- (IBAction)startStop:(id)sender
{
	...
	else
	{
        ...
        //加入群里，能接收到群里其他客户端的消息
        [udpSocket joinMulticastGroup:@"255.255.255.255" error:&error];
                                         ...
                                         withFilterContext:(id)filterContext
{
	...
	if (msg)
	{
		...
		*/
		...
		uint16_t port = 0;
		[GCDAsyncUdpSocket getHost:&host port:&port fromAddress:address];
		NSString *content = [NSString stringWithFormat:@"%@:%d,%@",host,port,msg];
    ...
    NSData *aData = [@"you_find_me" dataUsingEncoding: NSUTF8StringEncoding];
    
	[udpSocket sendData:aData toAddress:address withTimeout:-1 tag:0];
}
