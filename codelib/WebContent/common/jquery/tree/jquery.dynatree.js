...
DynaTreeNode.prototype = {
	initialize: function(parent, tree, data) {
		...
		if( tree.initMode == "cookie" ) {
			...
			if( tree.initActiveKey == this.data.value )
				tree.activeNode = this;
			if( tree.initFocusKey == this.data.value )
				tree.focusNode = this;
			this.bExpanded = ($.inArray(this.data.value, tree.initExpandedKeys) >= 0);
			this.bSelected = ($.inArray(this.data.value, tree.initSelectedKeys) >= 0);
		} else {
	...
	toString: function() {
		return "dtnode<" + this.data.value + ">: '" + this.data.text + "'";
	},
	...
	_getInnerHtml: function() {
		...
		// node name
		var tooltip = ( this.data && typeof this.data.tooltip == "string" ) ? " text='" + this.data.tooltip + "'" : "";
		res +=  "<a href='#'" + tooltip + ">" + this.data.text + "</a>";
		return res;
	},
	...
	render: function(bDeep, bHidden) {
		 ...
		 *   <span>S S S A</span> // Span contains graphic spans and text <a> tag 
		 *   <div>child1</div>
		 *   <div>child2</div>
		 * </div>
		 */
//		this.tree.logDebug("%o.render()", this);
		...
		if( ! this.div ) {
			...
			if( this.data.value )
				this.span.id = this.tree.options.idPrefix + this.data.value;
	...
	setLazyNodeStatus: function(lts) {
		switch( lts ) {
			...
			case DTNodeStatus_Loading:
				this._setStatusNode({
					text: this.tree.options.strings.loading,
//					icon: "ltWait.gif"
			...
			case DTNodeStatus_Error:
				this._setStatusNode({
					text: this.tree.options.strings.loadError,
//					icon: "ltError.gif"
...
//		this.tree.logDebug("dtnode.activate(): %o", this);
        ...
        if( opts.persist )
			$.cookie(opts.cookieId+"-active", this.data.value, opts.cookie);
...
/*		
//		this.tree.logDebug("toggleExpand("+this.data.text+")...");
		if( !this.childList && !this.data.isLazy )
			return;
		if( this.parent == null && this.tree.options.minExpandLevel>0 && this.bExpanded)
			return; // Prevent collapsing the root
		this._expand( ! this.bExpanded);
//		this.tree.logDebug("toggleExpand("+this.data.text+") done.");
*/
	},
...
//		this.tree.logDebug("dtnode.onKeydown(" + event.type + "): dtnode:" + this + ", charCode:" + event.charCode + ", valueCode: " + event.valueCode + ", which: " + event.which);
		var handled = true;
//		alert("valueDown" + event.which);
...
//			case 45: // '-'
	...
	onKeypress: function(event) {
		...
		// We don't process it, because IE and Safari don't fire valuepress for cursor values.
//		this.tree.logDebug("dtnode.onKeypress(" + event.type + "): dtnode:" + this + ", charCode:" + event.charCode + ", valueCode: " + event.valueCode + ", which: " + event.which);
	},
...
//		this.tree.logDebug("dtnode.onFocus(%o): %o", event, this);
		...
		if ( event.type=="blur" || event.type=="focusout" ) {
	        ...
	        if( opts.persist )
				$.cookie(opts.cookieId+"-focus", this.data.value, opts.cookie);
		}
...
//		this.tree.logDebug ("%o._addChildNode(%o), l=%o", this, dtnode, dtnode.getLevel());
	...
	append: function(obj) {
		...
		[
			{ text: "t1", isFolder: true, ... }
			{ text: "t2", isFolder: true, ...,
				children: [
					{text: "t2.1", ..},
					{..}
					]
...
DynaTree.prototype = {
	...
	initialize: function(divContainer, options) {
		...
		this.tnRoot = new DynaTreeNode(null, this, {text: this.options.text, value: "root"});
	...
	toString: function() {
		return "DynaTree '" + this.options.text + "'";
	},
	...
	_changeNodeList: function(mode, node, bAdd) {
		// Add or remove value from a value list and optionally write cookie.
		if( !node )
			return false;
...
//		this.logDebug("  -->: nodeList:%o", nodeList);
		if( this.options.persist ) {
			var valueList = $.map(nodeList, function(e,i){return e.data.value});
//			this.logDebug("_changeNodeList: write cookie <%s> = '%s'", cookieName, valueList.join("', '"));
			$.cookie(cookieName, valueList.join(","), this.options.cookie);
		} else {
//			this.logDebug("_changeNodeListCookie: %o", nodeList);
	...
	getNodeByKey: function(value) {
		// $("#...") has problems, if the value contains '.', so we use getElementById()
//		return $("#" + this.options.idPrefix + value).attr("dtnode");
		var el = document.getElementById(this.options.idPrefix + value);
	...
	activateKey: function(value) {
		var dtnode = this.getNodeByKey(value);
		if( !dtnode ) {
			this.activeNode = null;
			return null;
		}
	...
	selectKey: function(value, select) {
		var dtnode = this.getNodeByKey(value);
		if( !dtnode )
			return null;
 ...
 */
		$ulParent.find(">li").each(function() {
			...
			if( $liSpan.length ) {
				...
				text = $li.html();
				var iPos = text.search(/<ul/i);
				if( iPos>=0 )
					text = $.trim(text.substring(0, iPos));
				else
					text = $.trim(text);
//				self.logDebug("%o", text);
			}
			// Parse node options from ID, text and class attributes
			var data = {
				text: text,
			...
			if( $li.attr("text") )
				data.tooltip = $li.attr("text");
			if( $li.attr("id") )
				data.value = $li.attr("id");
...
*/
	...
	bind: function() {
		...
		$this.bind("click.dynatree dblclick.dynatree valuepress.dynatree valuedown.dynatree", function(event){
			var dtnode = __getNodeFromElement(event.target);
			
			if( !dtnode )
				return false;
//			dtnode.tree.logDebug("bind(" + event.type + "): dtnode:" + this + ", charCode:" + event.charCode + ", valueCode: " + event.valueCode + ", which: " + event.which);
...
$.ui.dynatree.defaults = {
	text: "Dynatree root", // Name of the root node.
	...
	valueboard: true, // Support valueboard navigation.
	...
	onKeydown: null, // null: generate valueboard navigation (focus, expand, activate).
...
//	onCollapse: null, // Callback(dtnode) when a node is collapsed.
	...
	idPrefix: "ui-dynatree-id-", // Used to generate node id's like <span id="ui-dynatree-id-<value>">.
...
$.ui.dynatree.nodedatadefaults = {
	text: null, // (required) Displayed name of the node (html is allowed here)
	value: null, // May be used with select(), find(), ...
