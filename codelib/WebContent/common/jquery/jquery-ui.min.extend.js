	...
	a.widget("ui.draggable", a.extend({}, a.ui.mouse, {
		...
		_getHandle : function(b) {
			var c = !this.options.handle
					...
					? true
					: false;
			a(this.options.handle, this.element).find("*").andSelf()
					.each(function() {
						if (this == b.target) {
							c = true
						}
			...
			for (var e = 0; e < this._proportionallyResizeElements.length; e++) {
				...
				if (!this.borderDif) {
					...
					this.borderDif = c.map(d, function(k, m) {
						var l = parseInt(k, 10) || 0, n = parseInt(h[m], 10)
								|| 0;
						return l + n
					})
				}
	...
	a.widget("ui.sortable", a.extend({}, a.ui.mouse, {
		...
		_intersectsWithSides : function(e) {
			var c = a.ui.isOverAxis(this.positionAbs.top
					+ this.offset.click.top, e.top + (e.height / 2), e.height), d = a.ui
					.isOverAxis(this.positionAbs.left + this.offset.click.left,
							e.left + (e.width / 2), e.width), b = this
					._getDragVerticalDirection(), f = this
					._getDragHorizontalDirection();
			if (this.floating && f) {
...
(function(a) {
	a.effects.explode = function(b) {
		return this.queue(function() {
			var k = b.options.pieces
					? Math.round(Math.sqrt(b.options.pieces))
					: 3;
			var e = b.options.pieces
					? Math.round(Math.sqrt(b.options.pieces))
					: 3;
				...
				for (var d = 0; d < e; d++) {
					h.clone().appendTo("body").wrap("<div></div>").css({
						...
						left : l.left
								+ d
								* (g / e)
								+ (b.options.mode == "show" ? (d - Math.floor(e
										/ 2))
										* (g / e) : 0),
						top : l.top
								+ f
								* (c / k)
								+ (b.options.mode == "show" ? (f - Math.floor(k
										/ 2))
										* (c / k) : 0),
						...
						left : l.left
								+ d
								* (g / e)
								+ (b.options.mode == "show" ? 0 : (d - Math
										.floor(e / 2))
										* (g / e)),
						top : l.top
								+ f
								* (c / k)
								+ (b.options.mode == "show" ? 0 : (f - Math
										.floor(k / 2))
										* (c / k)),
...
(function(a) {
	...
	a.effects.scale = function(b) {
		return this.queue(function() {
			...
			var h = parseInt(b.options.percent, 10)
					|| (parseInt(b.options.percent, 10) == 0 ? 0 : (j == "hide"
							? 0
							: 100));
	...
	a.effects.size = function(b) {
		return this.queue(function() {
			var c = a(this), n = ["position", "top", "left", "width", "height",
					"overflow", "opacity"];
				...
				c.find("*[width]").each(function() {
					...
					if (l.from.y != l.to.y) {
						child.from = a.effects.setTransition(child, k,
								l.from.y, child.from);
						child.to = a.effects.setTransition(child, k, l.to.y,
								child.to)
					}
					if (l.from.x != l.to.x) {
						child.from = a.effects.setTransition(child, f,
								l.from.x, child.from);
						child.to = a.effects.setTransition(child, f, l.to.x,
								child.to)
					}
		...
		return this.queue(function() {
			var f = a(this), h = a(b.options.to), e = h.offset(), g = {
				...
				width : h.innerWidth()
					...
					.appendTo(document.body).addClass(b.options.className)
					.css({
						top : d.top,
						left : d.left,
		...
		_generateHTML : function(inst) {
			...
			if (maxDate) {
				var maxDraw = this._daylightSavingAdjust(new Date(maxDate
						.getFullYear(), maxDate.getMonth() - numMonths[1] + 1,
						maxDate.getDate()));
		...
		_init : function() {
			...
			var l = this, m = this.options, j = m.title || this.originalTitle
					...
					.css({
						...
						overflow : "hidden",
						zIndex : m.zIndex
					}).attr("tabIndex", -1).css("outline", 0).keydown(function(
							n) {
			...
			if (d) {
				c.each(g, function(h, i) {
					c('<button type="button"></button>')
							.addClass("ui-state-default ui-corner-all").text(h)
							.click(function() {
			...
			if (this.instances.length === 0) {
				setTimeout(function() {
					c(document).bind(c.ui.dialog.overlay.events, function(f) {
						var g = c(f.target).parents(".ui-dialog").css("zIndex")
								|| 0;
...
//						.addClass("ui-tabs-panel ui-widget-content ui-corner-bottom");
				...
				this.list.css({'position':'relative','padding-left':'20px','padding-right':'20px'});
				this.list.prepend($('<li><span class="ui-tab-leftscroll">&nbsp;</span></li>').css({cursor:'pointer',position:'absolute',left:'0px',top:'0px'}).click(
					function(){
						var _li=p.list.find('li');
						var _hidden=_li.filter(':hidden').length;
						if(_hidden==0)return;
						_li.eq(_hidden).show();
					}
				));
				this.list.append($('<li><span class="ui-tab-rightscroll">&nbsp;</span></li>').css({cursor:'pointer',position:'absolute',right:'0px',top:'0px'}).click(
					function(){
						var _li=p.list.find('li');
						var _hidden=_li.filter(':hidden').length;
						if(_hidden>=(_li.length-3))return;
						var _cls=_li.eq(_hidden+1).hasClass('ui-tabs-selected');
						_li.eq(_hidden+1).hide();
						if(_cls==true){
							_li.eq(_hidden+2).find('a').trigger('click');
						}
					}
				));
				this.list.addClass('ui-tabs-nav ui-helper-reset ui-helper-clearfix ui-widget-header ui-corner-all');
				this.lis.addClass('ui-state-default ui-corner-top');
				this.list.css('height','30px').css('overflow','hidden');
				...
				if (d.selected >= 0 && this.anchors.length) {
					...
					p.element.queue("tabs", function() {
						p._trigger("show", null, p._ui(p.anchors[d.selected],
								p.panels[d.selected]))
					});
