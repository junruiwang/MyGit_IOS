...
(function () {
	...
	f.uniqueId.lastId = 0;
	f.policies = {options:{product:["product", "v3"], insertMode:["insertMode", "replace"], safeMode:["safeMode", true], overlayButton:["overlayButton", undefined]}, attributes:{lang:["lang", "EN"], "class":["className", "FusionCharts"]}, width:["width", "400"], height:["height", "300"], src:["swfUrl", ""], __state:{}};
	...
	f.extend({id:"FusionCharts", version:[3, 2, 1, "release", 1750], items:{}, options:{}, getObjectReference:function (a) {
...
(function () {
	...
	var j = {listeners:{}, lastEventId:0, addListener:function (a, b, c) {
		if (a instanceof Array) {
		...
		a = a.toLowerCase();
		var d = {eventType:a, eventId:(j.lastEventId += 1), sender:(typeof b === "string" ? f.core.items[b] : b), stopPropagation:function () {
	...
	f.extend({addEventListener:f.addEventListener, removeEventListener:f.removeEventListener}, false);
	f.extend({addEventListener:function (a, b) {
...
(function () {
	...
	var j = "text";
	var k = {type:"TypeException", range:"ValueRangeException", impl:"NotImplementedException", param:"ParameterException", run:"RuntimeException", comp:"DesignTimeError", "undefined":"UnspecifiedException"};
	var l = function (a, b, c, d, e, f) {
		...
		var h = {id:b, nature:k[c], source:"FusionCharts" + d, message:g};
		i.raiseEvent(f, h, a);
	...
	var m = {outputHelpers:{"text":function (e, a) {
	...
	i.extend({debugMode:{outputFormat:function (a) {
...
(function () {
	...
	var h = {"undefined":{render:g, update:g, resize:g, config:g, policies:{}}}, store = {};
	f.renderer = {register:function (a, b) {
	...
	f.extend({setCurrentRenderer:f.renderer.setDefault, render:function () {
...
(function () {
	...
	g.ajax = (function () {
		...
		var d = {x:window.XMLHttpRequest && (window.location.protocol !== "file:" || !window.ActiveXObject) ? function () {
	...
	g.ajax.error = function (a, x, b, c, e) {
		var d = {source:b, url:a, xmlHttpRequestObject:x, error:e, httpStatus:(x && x.status) ? x.status : -1};
	...
	g.extend({setChartDataUrl:function (c, d, e) {
		...
		g.raiseEvent("DataLoadRequested", {source:"XmlHttpRequest", url:c, dataFormat:f, cancelDataLoadRequest:function () {
			cancelDLRFlag = true;
		...
		if (cancelDLRFlag === true) {
			g.raiseEvent("DataLoadRequestCancelled", {source:"XmlHttpRequest", url:c, dataFormat:f}, obj);
			return;
		}
		this.options.dataSource = c;
		xStore[this.id] = g.ajax.get(decodeURIComponent(c), function (a, x) {
			var b = false;
			g.raiseEvent("DataLoadRequestCompleted", {source:"XmlHttpRequest", url:c, data:a, dataFormat:f, cancelDataLoad:function () {
				b = true;
				this.cancelDataLoad = function () {
					return false;
				};
				return true;
			}, xmlHttpRequestObject:x}, obj);
			if (b !== true) {
				obj.setChartData(a, f, e);
			} else {
				g.raiseEvent("DataLoadCancelled", {source:"XmlHttpRequest", url:c, dataFormat:f, xmlHttpRequestObject:x}, obj);
			}
		...
		eventArgs = {dataFormat:d, dataSource:a, dataError:parseResult.error, data:parseResult.data, cancelDataUpdate:function () {
			cancelDUFlag = true;
...
(function () {
	...
	var j = {vled:"realtimeverticalled"};
	...
	f.addEventListener("internal.DOMElementCreated", function (a, b) {
		...
		var c = {options:true, vars:true, attributes:true, params:true, src:true, ref:true, constructor:true, setDataXML:true, setDataURL:true, hasRendered:true, getXML:true, getDataAsCSV:true, print:true, exportChart:true, signature:true, link:true};
...
(function () {
	...
	d.extend({setChartAttribute:function (a) {
		if (arguments.length > 1 && typeof a === "string") {
			var b = arguments[0];
			a = {};
			a[b] = arguments[1];
...
(function () {
	...
	var z = y.swfobject = function () {
		var w = "undefined", OBJECT = "object", SHOCKWAVE_FLASH = "Shockwave Flash", SHOCKWAVE_FLASH_AX = "ShockwaveFlash.ShockwaveFlash", FLASH_MIME_TYPE = "application/x-shockwave-flash", EXPRESS_INSTALL_ID = "SWFObjectExprInst", ON_READY_STATE_CHANGE = "onreadystatechange", win = window, doc = document, nav = navigator, plugin = false, domLoadFnArr = [main], regObjArr = [], objIdArr = [], listenersArr = [], storedAltContent, storedAltContentId, storedCallbackFn, storedCallbackObj, isDomLoaded = false, isExpressInstallActive = false, dynamicStylesheet, dynamicStylesheetMedia, autoHideShow = true, ua = function () {
			...
			return {w3:b, pv:playerVersion, wk:webkit, ie:ie, win:windows, mac:mac};
		}(), onDomLoad = function () {
			if (!ua.w3) {
				return;
			}
		...
		function showExpressInstall(a, b, c, d) {
			...
			storedCallbackObj = {success:false, id:c};
	...
	var D = function (e, a) {
		...
		if (b.ref && typeof b.ref.dataInvokedOnSWF === "function" && b.ref.dataInvokedOnSWF() && typeof b.ref.getXML === "function") {
			...
			b.setChartData(window.unescape(e.sender.ref.getXML({escaped:true})), FusionChartsDataFormats.XML, true);
	...
	window.__fusioncharts_dimension = (function () {
		...
		return function (a) {
			var b, parent;
			return !((b = y.core(a)) instanceof y.core && b.ref && (parent = b.ref.parentNode)) ? {} : {width:parent.offsetWidth * (c.test(b.width) ? parseInt(b.width, 10) / 100 : 1), height:parent.offsetHeight * (c.test(b.height) ? parseInt(b.height, 10) / 100 : 1)};
		};
	}());
	...
	y.addEventListener("BeforeInitialize", function (a) {
		...
		y.extend(b.flashVars, {registerWithJS:"1", chartWidth:b.width, chartHeight:b.height, InvalidXMLText:"Invalid data."});
	...
	var E = {dataFormat:"xml", init:function () {
	}, policies:{params:{scaleMode:["scaleMode", "noScale"], scale:["scaleMode", "noScale"], wMode:["wMode", "opaque"], menu:["menu", undefined], bgColor:["bgColor", undefined], allowScriptAccess:["allowScriptAccess", "always"], quality:["quality", "best"], swLiveConnect:["swLiveConnect", undefined], base:["base", undefined], align:["align", undefined], salign:["sAlign", undefined]}, flashVars:{lang:["lang", "EN"], debugMode:["debugMode", undefined], animation:["animate", undefined]}, options:{autoInstallRedirect:["autoInstallRedirect", false]}}, render:function (a, b) {
	...
	E.prototype = {getSWFHTML:function () {
...
(function () {
	...
	FCC.init = function () {
		...
		}}, supportedStyle:{font:function (c, d, e) {
			var f, x, map = {font:m("font-family"), size:m("font-size", "px"), color:m("color", undefined, "#"), align:function (a, b) {
				b.align = a;
			}, bgColor:m("background-color", undefined, "#"), borderColor:m("border-color", undefined, "#"), isHTML:"", leftMargin:m("margin-left", "px"), letterSpacing:m("letter-spacing", "px"), bold:function (a, b) {
		...
		FCC.prototype = {setTransparent:function (a) {
			if (!this.jsVars) {
				this.jsVars = {};
			}
			if (typeof a !== "boolean" && a !== null) {
		...
		function createSeries(a, b, c, d, e, f, g, h, i) {
			return {data:[], FCtype:a, color:b, type:c, name:d ? d : " ", yAxis:e ? 1 : undefined, showvalue:f, lineColor:g, lineWidth:h, marker:i};
		}
		function parseStr(a) {
			if (typeof a === "string") {
		...
		function showChartMessage(a) {
			...
			var c = M.extend({show:true}, this.FusionCharts.link.parent.options.overlayButton);
		...
		var t = function (a) {
			var x, psum = 0, pareto = {yAxis:1, data:[], type:"line", color:"rgba(00,00,00,1)", FCtype:1.5}, sum = 0;
			if (a instanceof Array) {
				for (x = 0; x < a.length; x += 1) {
					sum += a[x].y;
				}
				for (x = 0; x < a.length; x += 1) {
					psum += a[x].y;
					pareto.data.push({y:Math.round((psum / sum) * 10000) / 100});
				}
			}
			return pareto;
		};
		...
		var v = function (a, b, d) {
			d = d === undefined ? 1 : d;
			var e = {formatnumber:1, formatnumberscale:1, defaultnumberscale:"", numberscaleunit:"K,M", numberscalevalue:"1000,1000", numberprefix:"", numbersuffix:"", decimalseparator:".", thousandseparator:",", indecimalseparator:"", inthousandseparator:"", decimals:"", forcedecimals:"0", yaxisvaluedecimals:"", sformatnumber:"1", sformatnumberscale:"0", sdefaultnumberscale:"", snumberscaleunit:"K,M", snumberscalevalue:"1000,1000", snumberprefix:"", snumbersuffix:"", sdecimals:"", sforcedecimals:"0", syaxisvaluedecimals:"0"};
			...
			if ((a === 2 && this.link) || (a === 1 && this.options.chart.link)) {
				...
				switch (arr[0].toLowerCase()) {
				  ...
				  case "newchart":
					...
					M.raiseEvent("LinkedChartInvoked", {linkType:d, data:data}, sender);
					break;
				  default:
					arr = c.split(":");
		...
		function createChart(a, b) {
			...
			FCC.items[id] = {"baseObj":b, "FCCObj":d, "chartType":c};
		}
		var I = function (c, d) {
			var e;
		...
		var K = function (a) {
			return {FCconf:{negative:false, stackValue:[]}, chart:{alignTicks:false, renderTo:"", ignoreHiddenSeries:false, events:{click:function () {
				...
				return parseStr(x);
			}}, xAxis:{labels:{rotation:-25, style:{}, formatter:function () {
				return parseStr(typeof this.value === "string" ? this.value : "");
			}, align:"right"}, categories:[], plotLines:[], plotBands:[], title:{style:{}, text:""}}, yAxis:[{startOnTick:false, endOnTick:false, title:{style:{}, text:""}, labels:{style:{}, formatter:function () {
				return parseStr(v(this.value, a.chart, 1));
			}}, plotBands:[], plotLines:[]}, {gridLineWidth:0, startOnTick:false, endOnTick:false, title:{style:{}, text:""}, labels:{style:{}, enabled:false, formatter:function () {
				return parseStr(v(this.value, a.chart, 2));
			}}, opposite:true}], exporting:{buttons:{exportButton:{}, printButton:{enabled:false}}}};
		};
		function addVline(a, b, c, d, e) {
			a.xAxis.plotLines.push({color:convertColor(b || "#444444", c || 100), width:d ? d : 1, value:e});
		}
				...
				for (x = 0; x < b.dataset.length; x += 1) {
					...
					if (b.dataset[x].dataset && b.dataset[x].dataset instanceof Array) {
						for (y = 0; y < b.dataset[x].dataset.length; y += 1) {
							newSeries = {FCtype:c, type:b.dataset[x].dataset[y].renderas ? FCFCC.combi[b.dataset[x].dataset[y].renderas.toLowerCase()] : undefined, name:b.dataset[x].dataset[y].seriesname ? b.dataset[x].dataset[y].seriesname : " ", data:[], color:convertColor(b.dataset[x].color || i(count_set), b.dataset[x].dataset[y].alpha || 100), yAxis:b.dataset[x].dataset[y].parentyaxis == "S" ? 1 : undefined, showvalue:b.dataset[x].dataset[y].showvalues, mColum:x, mStack:y};
							if (b.dataset[x].dataset[y].data instanceof Array) {
								for (z = 0; z < max_cat; z += 1) {
									...
									newSeries.data.push({MWidth:pointWidth, MX:(z * catWidth) + ((x + 1) * pointWidth), MY:tempheight[z], y:pointValue, link:b.dataset[x].dataset[y].data[z].link, id:h, color:b.dataset[x].dataset[y].data[z].color ? convertColor(b.dataset[x].dataset[y].data[z].color || i(count_set), b.dataset[x].dataset[y].data[z].alpha || 100) : undefined});
								}
							}
							a.push(newSeries);
							count_set += 1;
			  ...
			  case 7.5:
				...
				if (c.dataset) {
					...
					for (x = 0; x < c.dataset.length; x += 1) {
						...
						z.marker = {symbol:(d == 7.5) ? "circle" : w(parseInt(c.dataset[x].anchorsides, 10)), states:{hover:{enabled:false}}};
						count_set += 1;
						if (c.dataset[x].data) {
		...
		function createText(a, b, c) {
			return {html:a, style:{left:c + "px", top:b + "px"}};
		}
		function convertMarimeko(a, b, c, d, e) {
			...
			for (y = 0; y < a.series.length; y += 1) {
				...
				for (z = 0; z < a.series[y].data.length; z += 1) {
					if (typeof a.series[y].data[z] !== "object") {
						a.series[y].data[z] = {y:a.series[y].data[z]};
					}
		...
		function covertToFCC(a, b, c, d, e, f) {
			...
			g.xAxis.labels.style = {fontFamily:c.chart.outcnvbasefont || c.chart.basefont || "Verdana", fontSize:(c.chart.outcnvbasefontsize || c.chart.basefontsize || "10") + "px", color:c.chart.outcnvbasefontcolor || c.chart.basefontcolor || "#555555"};
			g.yAxis[0].labels.style = {fontFamily:c.chart.outcnvbasefont || c.chart.basefont || "Verdana", fontSize:(c.chart.outcnvbasefontsize || c.chart.basefontsize || "10") + "px", color:c.chart.outcnvbasefontcolor || c.chart.basefontcolor || "#555555"};
			g.yAxis[1].labels.style = {fontFamily:c.chart.outcnvbasefont || c.chart.basefont || "Verdana", fontSize:(c.chart.outcnvbasefontsize || c.chart.basefontsize || "10") + "px", color:c.chart.outcnvbasefontcolor || c.chart.basefontcolor || "#555555"};
			g.legend.itemStyle = {fontFamily:c.chart.outcnvbasefont || c.chart.basefont || "Verdana", fontSize:(c.chart.outcnvbasefontsize || c.chart.basefontsize || "10") + "px", color:c.chart.outcnvbasefontcolor || c.chart.basefontcolor || "#555555"};
			g.plotOptions.series.dataLabels.style = {fontFamily:c.chart.basefont || "Verdana", fontSize:(c.chart.basefontsize || "10") + "px", color:c.chart.basefontcolor || "#555555"};
			g.tooltip.style = {fontFamily:c.chart.basefont || "Verdana", fontSize:(c.chart.basefontsize || "10") + "px", color:c.chart.basefontcolor || "#555555"};
			...
			if (c.chart.caption) {
				...
				g.chart.margin[0] = 35;
				g.title.style = {fontFamily:c.chart.outcnvbasefont || c.chart.basefont || "Verdana", fontSize:"13px", "font-weight":"bold", color:c.chart.outcnvbasefontcolor || c.chart.basefontcolor || "#555555"};
			}
			if (c.chart.subcaption) {
				g.subtitle.text = parseStr(c.chart.subcaption);
				g.chart.margin[0] = 55;
				g.subtitle.style = {fontFamily:c.chart.outcnvbasefont || c.chart.basefont || "Verdana", fontSize:"11px", "font-weight":"bold", color:c.chart.outcnvbasefontcolor || c.chart.basefontcolor || "#555555"};
			}
			if (c.chart.xaxisname) {
				g.xAxis.title.text = parseStr(c.chart.xaxisname);
				g.xAxis.title.margin = g.chart.margin[2];
				g.chart.margin[2] += 20;
				g.xAxis.title.style = {fontFamily:c.chart.outcnvbasefont || c.chart.basefont || "Verdana", fontSize:(c.chart.outcnvbasefontsize || c.chart.basefontsize || "10") + "px", "font-weight":"bold", color:c.chart.outcnvbasefontcolor || c.chart.basefontcolor || "#555555"};
			}
			if (y == 5 || y == 5.5 || y == 1.5) {
				if (c.chart.pyaxisname) {
					...
					g.chart.margin[3] += 20;
					g.yAxis[0].title.style = {fontFamily:c.chart.outcnvbasefont || c.chart.basefont || "Verdana", fontSize:(c.chart.outcnvbasefontsize || c.chart.basefontsize || "10") + "px", "font-weight":"bold", color:c.chart.outcnvbasefontcolor || c.chart.basefontcolor || "#555555"};
				}
				if (c.chart.syaxisname) {
					g.yAxis[1].title.text = parseStr(c.chart.syaxisname);
					g.yAxis[1].title.margin = g.chart.margin[1];
					g.chart.margin[1] += 20;
					g.yAxis[1].title.style = {fontFamily:c.chart.outcnvbasefont || c.chart.basefont || "Verdana", fontSize:(c.chart.outcnvbasefontsize || c.chart.basefontsize || "10") + "px", "font-weight":"bold", color:c.chart.outcnvbasefontcolor || c.chart.basefontcolor || "#555555"};
				}
			} else {
					...
					if (c.trendlines[x].line) {
						for (z = 0; z < c.trendlines[x].line.length; z += 1) {
							if (c.trendlines[x].line[z].istrendzone == "1") {
								g.yAxis[0].plotBands.push({color:convertColor(c.trendlines[x].line[z].color || "333333", c.trendlines[x].line[z].alpha || 99), from:c.trendlines[x].line[z].startvalue ? c.trendlines[x].line[z].startvalue : 0, to:c.trendlines[x].line[z].endvalue});
							}
							g.yAxis[0].plotLines.push({color:convertColor(c.trendlines[x].line[z].color || "333333", c.trendlines[x].line[z].alpha || 99), value:c.trendlines[x].line[z].startvalue ? c.trendlines[x].line[z].startvalue : 0, width:c.trendlines[x].line[z].thickness ? c.trendlines[x].line[z].thickness : 1});
						}
					}
				}
			}
					...
					if (c.vtrendlines[x].line) {
						for (z = 0; z < c.vtrendlines[x].line.length; z += 1) {
							if (c.vtrendlines[x].line[z].istrendzone !== "0") {
								g.xAxis.plotBands.push({color:convertColor(c.vtrendlines[x].line[z].color || "333333", c.vtrendlines[x].line[z].alpha || 99), from:c.vtrendlines[x].line[z].startvalue ? c.vtrendlines[x].line[z].startvalue : 0, to:c.vtrendlines[x].line[z].endvalue});
							}
							g.xAxis.plotLines.push({color:convertColor(c.vtrendlines[x].line[z].color || "333333", c.vtrendlines[x].line[z].alpha || 99), value:c.vtrendlines[x].line[z].startvalue ? c.vtrendlines[x].line[z].startvalue : 0, width:c.vtrendlines[x].line[z].thickness ? c.vtrendlines[x].line[z].thickness : 1});
						}
					}
				}
			}
			...
			g.yAxis[0].plotLines.push({color:convertColor(c.chart.zeroplanecolor || "717170", c.chart.zeroplanealpha || 80), value:0, width:c.chart.zeroplanethickness || 2});
		...
		FCC.render = function (b, c) {
			var d, sender = this, events = {loaded:"FC_Loaded", dataloaded:"FC_DataLoaded", rendered:"FC_Rendered", drawcomplete:"FC_DrawComplete"};
			if (FCC.isReady()) {
		...
		FCC.update = function (b, c) {
			var d = this.id, __containerId = this.options.containerElementId, sender = this, HCJson, events = {dataloaded:"FC_DataLoaded", drawcomplete:"FC_DrawComplete"};
		...
		L.prototype = {"LoadDataErrorText":"Error in loading data.", "XMLLoadingText":"Retrieving Data. Please Wait", "InvalidXMLText":"Invalid data.", "NoDataText":"No data to display.", "ReadingDataText":"Reading Data. Please Wait", "ChartNotSupported":"Chart Has No Javascript FallBack"};
	...
	M.extend({"_fallbackJSChartWhenNoFlash":function () {
...
(function () {
	...
	var c = function (a) {
		return {data:a, error:undefined};
	};
	b.addDataHandler("XML", {encode:c, decode:c});
}());
if (!this.JSON) {
	this.JSON = {};
}
...
(function () {
	...
	var l = (function () {
		...
		return function (a) {
			delete h.errorObject;
			var b = h(a);
			return {data:b, error:h.errorObject};
		};
	}());
	var m = (function () {
		var d = {items:{explode:{data:"set"}, attr:{chart:{chart:"chart", graph:"chart"}, graph:{graph:"graph", chart:"graph"}, map:{map:"map"}, linkedchart:{chart:"chart", graph:"graph", map:"map"}}, group:{styles:{definition:"style", application:"apply"}}}, qualify:function (a, b, c) {
			c = c.toLowerCase();
		...
		return function (a) {
			...
			return {data:b, error:f.errorObject};
		};
	}());
	j.addDataHandler("JSON", {encode:function (a) {
		return m(a);
	}, decode:function (a) {
		return l(a);
	}});
...
(function () {
	...
	f.extend({configureLink:function (a, b) {
		if (a instanceof Array) {
			for (var i = 0; i < a.length; i += 1) {
	...
	f.addEventListener("LinkedChartInvoked", function (a, b) {
		var c = a.sender, param = c.clone({dataSource:b.data, dataFormat:b.linkType === "URL" ? FusionChartsDataFormats.XMLURL : b.linkType, link:new h(c.link.root, c)}, true);
		if (c.args && parseInt(c.args.animate, 10) !== 0) {
			delete param.animate;
		}
		f.extend(param, c.link.configuration());
		f.raiseEvent("BeforeLinkedItemOpen", {level:c.link.level}, c.link.root);
		...
		f.raiseEvent("LinkedItemOpened", {level:c.link.level, item:d}, c.link.root);
	});
	f.addEventListener("OverlayButtonClick", function (a) {
		var b = a.sender, level = b.link.level - 1, parent = b.link.parent;
		f.raiseEvent("BeforeLinkedItemClose", {level:level, item:b}, b.link.root);
		b.dispose();
		f.raiseEvent("LinkedItemClosed", {level:level}, b.link.root);
	...
	f.addEventListener("Loaded", function (a) {
		...
		var c = f.extend({show:true}, b.link.parent.options.overlayButton);
...
(function () {
	...
	var k = {enabled:false, invokeCSS:true, processPollInterval:2000, message:"Chart is being prepared for print.", useExCanvas:false, bypass:false};
	var l = {getCanvasElementOf:function (a, b, c) {
	...
	var m = {styles:{print:"canvas.FusionCharts{display:none;}@media print{object.FusionCharts{display:none;}canvas.FusionCharts{display:block;}}", error:"canvas.FusionCharts{display:none;}", normal:""}, cssNode:undefined, invoke:function (a) {
	...
	var n = function (a) {
		...
		if (!activeItems[a.sender.id]) {
			...
			activeCount += 1;
			if (activeCount === 1) {
				j.raiseEvent("PrintReadyStateChange", {ready:false, bypass:k.bypass}, a.sender);
			}
		}
		try {
			w = b.offsetWidth;
		...
		try {
			if (l.drawRLE(l.getCanvasElementOf(a.sender.ref, b.width, b.height), b.stream, b.width, b.height, b.bgColor) === true) {
				if (activeItems[a.sender.id]) {
					...
					activeCount -= 1;
					if (activeCount === 0) {
						j.raiseEvent("PrintReadyStateChange", {ready:true, bypass:k.bypass}, a.sender);
					}
				}
			}
		}
	...
	queueTrigger = function (a) {
		...
		for (var b in queuedItems) {
			n({sender:queuedItems[b]}, {});
			delete queuedItems[b];
		}
	};
	j.extend({printManager:{configure:function (a) {
		j.extend(k, a || {});
	}, isReady:function () {
		if (k.bypass) {
			return true;
		...
		if ("\v" === "v" || j.renderer.currentRendererName() !== "flash" || typeof document.createElement("canvas").getContext !== "function") {
			k.bypass = true;
			j.raiseEvent("PrintReadyStateChange", {ready:true, bypass:k.bypass});
