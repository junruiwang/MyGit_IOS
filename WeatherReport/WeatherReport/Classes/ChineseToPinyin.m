...
@interface ChineseToPinyin ()

+ (NSString *)findLetter:(NSInteger) nCode;
...
+ (NSString *) pinyinFromChiniseString:(NSString *)string
{
    ...
    for (int i=0; i< iLen; i++)
    {
		...
		NSString * strRes = [self findLetter:nCode];
...
+ (char) sortSectionTitle:(NSString *)string
{
	...
	{	
		   ...
		   ([string characterAtIndex:0] > 96 &&  [string characterAtIndex:0] < 123 ) )
		{
			...
			cLetter = [self pinyinFirstLetter:((unsigned short)[string characterAtIndex:0])];
