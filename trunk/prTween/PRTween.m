...
+ (id)periodWithStartValue:(CGFloat)aStartValue endValue:(CGFloat)anEndValue duration:(CGFloat)duration {
    PRTweenPeriod *period = [[PRTweenPeriod alloc] init];
    period.startValue = aStartValue;
...
+ (id)periodWithStartValue:(NSValue*)aStartValue endValue:(NSValue*)anEndValue duration:(CGFloat)duration {
    PRTweenLerpPeriod *period = [[[self class] alloc] init];
...
- (void)dealloc {
    NSLog(@"PRTweenOperation dealloc");
    /*
    @try {
    ...
    @try {
        [self removeObserver:[PRTween sharedInstance] forKeyPath:@"period.tweenedLerp"];
    } @catch (id exception) {
...
+ (PRTweenOperation *)tween:(id)object property:(NSString*)property from:(CGFloat)from to:(CGFloat)to duration:(CGFloat)duration timingFunction:(PRTweenTimingFunction)timingFunction target:(NSObject*)target completeSelector:(SEL)selector {
    ...
    PRTweenOperation *operation = [[[PRTweenOperation alloc] init] autorelease];
    //operation.target=target;//jeff
...
complete:
    [tweenOperations addObject:(PRTweenOperation *)operation];
    return operation;
...
- (PRTweenOperation*)addTweenPeriod:(PRTweenPeriod *)period 
    ...
    PRTweenOperation *tweenOperation = [[[PRTweenOperation alloc] init] autorelease];
...
+(void)removeTweenOperation:(PRTweenOperation*)tweenOperation{
    [[PRTween sharedInstance] removeTweenOperation:tweenOperation];
}
+ (SEL)setterFromProperty:(NSString *)property {
...
- (void)update {
    ...
    //NSLog(@"tweenOperations::%@",tweenOperations);
    
    for (PRTweenOperation *tweenOperation in tweenOperations) {
    //for (NSInteger i=0;i<[tweenOperations count];i++) {
        //PRTweenOperation *tweenOperation=[tweenOperations objectAtIndex:i];
    ...
    for (PRTweenOperation *tweenOperation in expiredTweenOperations) {
        ...
        if (tweenOperation.completeSelector){
            //tweenOperation.target////////////jeff
            [tweenOperation.target performSelector:tweenOperation.completeSelector withObject:nil afterDelay:0];
