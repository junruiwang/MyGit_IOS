...
typedef void(^SuccessBlock)(UIImage *image, NSURL *url);
...
#if NS_BLOCKS_AVAILABLE
- (void)downloadWithURL:(NSURL *)url delegate:(id)delegate options:(SDWebImageOptions)options success:(void (^)(UIImage *image, NSURL *url))success failure:(void (^)(NSError *error))failure
{
    ...
    for (NSInteger idx = (NSInteger)[downloaders count] - 1; idx >= 0; idx--)
    {
        ...
        if (aDownloader == downloader)
        {
            ...
            if (image)
            {
                ...
                if (self.successBlock)
                {
                    self.successBlock(image, downloader.url);
                }
#endif
            }
            else
            ...
            if([downloaders count] > uidx)
                [downloaders removeObjectAtIndex:uidx];
            if([downloadDelegates count] > uidx)
