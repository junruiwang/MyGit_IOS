...
- (NSArray *)calculateDaysInSelectedMonth
{
    ...
    const unsigned int numDays = [self.baseDate cc_numberOfDaysInMonth];
    NSDateComponents *c = [self.baseDate cc_componentsForMonthDayAndYear];
    for (unsigned int i = 1; i < numDays + 1; i++)
    {   [days addObject:[KalDate dateForDay:i month:c.month year:c.year]];  }
...
- (NSArray *)calculateDaysInFirstWeekOfFollowingMonth
{
    ...
    const unsigned int numPartialDays = [self numberOfDaysInFollowingPartialWeek];
    
    for (unsigned int i = 1; i < numPartialDays + 1; i++)
    {   [days addObject:[KalDate dateForDay:i month:c.month year:c.year]];  }
