...
- (NSString *)XMLString {
  ...
  if (xmlNode_ != NULL) {
    ...
    if (buff) {
      ...
      const int result = xmlNodeDump(buff, doc, xmlNode_, level, format);
    ...
    if (xpathCtx) {
      ...
      if (namespaces) {
        ...
        for (NSString *prefix in namespaces) {
          ...
          const int result = xmlXPathRegisterNs(xpathCtx, prefixChars, uriChars);
          if (result != 0) {
#if DEBUG
        ...
        if (nsNodePtr != NULL) {
          for (xmlNsPtr nsPtr = nsNodePtr->ns; nsPtr != NULL; nsPtr = nsPtr->next) {
            ...
            const int result = xmlXPathRegisterNs(xpathCtx, prefix, nsPtr->href);
            if (result != 0) {
#if DEBUG
      ...
      if (xpathObj) {
        ...
        if (nodeSet) {
          ...
          for (unsigned int index = 0; index < nodeSet->nodeNr; index++) {
...
- (NSString *)description {
  const int nodeType = (xmlNode_ ? (int)xmlNode_->type : -1);
...
- (const unsigned int)hash {
  return (const unsigned int) (void *) [GDataXMLNode class];
}
...
{
    if ([self elementsForName:name].count > 0) {
        return [self elementsForName:name][0];
    }
    return nil;
}
  ...
  if (nodeToFix->ns == NULL) {
    ...
    if (localName != NULL) {
      if (prefix != NULL) {
        ...
        const int prefixLen = xmlStrlen(prefix);
        if (prefixLen > 2
            && prefix[0] == '{'
            && prefix[prefixLen - 1] == '}') {
...
static Boolean StringCacheKeyEqualCallBack(const void *str1, const void *str2) {
  ...
  const int result = xmlStrcmp(str1, str2);
  return (result == 0);
}
