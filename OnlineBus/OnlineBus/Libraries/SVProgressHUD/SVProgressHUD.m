...
+ (SVProgressHUD*)sharedView
{
    ...
    return [self sharedViewOnlyInstance:screenRect];
}

+ (SVProgressHUD*)sharedView:(CGRect)frame
{
    return [self sharedViewOnlyInstance:frame];
}

+ (SVProgressHUD*)sharedViewOnlyInstance:(CGRect)frame
{
    static dispatch_once_t once;
    static SVProgressHUD *sharedView;
    dispatch_once(&once, ^ { sharedView = [[SVProgressHUD alloc] initWithFrame:frame]; });
    return sharedView;
}
...
{
    [[SVProgressHUD sharedView:frame] showWithStatus:nil maskType:SVProgressHUDMaskTypeNone networkIndicator:NO];
}
...
+ (void)showWithStatus:(NSString*)status frameSize:(CGRect)frame
{
    [[SVProgressHUD sharedView:frame] showWithStatus:status maskType:SVProgressHUDMaskTypeNone networkIndicator:NO];
}
...
+ (void)showWithStatus:(NSString*)status maskType:(SVProgressHUDMaskType)maskType frameSize:(CGRect)frame
{
    [[SVProgressHUD sharedView:frame] showWithStatus:status maskType:maskType networkIndicator:NO];
}
...
+ (void)showWithMaskType:(SVProgressHUDMaskType)maskType frameSize:(CGRect)frame
{
    [[SVProgressHUD sharedView:frame] showWithStatus:nil maskType:maskType networkIndicator:NO];
}
...
- (void)positionHUD:(NSNotification*)notification
{
    ...
    CGRect orientationFrame = self.windowFrame;
...
- (void)showWithStatus:(NSString*)string maskType:(SVProgressHUDMaskType)hudMaskType networkIndicator:(BOOL)show
{
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        self.hudView.bounds = CGRectMake(0, 0, hudWidth, hudHeight);
        self.closeButton.center = CGPointMake(CGRectGetWidth(self.hudView.bounds)-14, 14);
        ...
        if(self.alpha != 1)
        {
            ...
            [self.hudView setTransform:CGAffineTransformScale(self.hudView.transform, 1.3, 1.3)];
...
- (UIWindow *)overlayWindow
{
    if(!overlayWindow)
    {
        overlayWindow = [[UIWindow alloc] initWithFrame:self.windowFrame];
...
- (UIButton *)closeButton
{
    if (closeButton == nil)
    {
        ...
        [closeButton setImage:[UIImage imageNamed:@"SVProgressHUD.bundle/error.png"] forState:UIControlStateNormal];
...
- (CGFloat)visibleKeyboardHeight
{
    ...
    for (__strong UIView *possibleKeyboard in [keyboardWindow subviews])
    {
        ...
        {   foundKeyboard = possibleKeyboard;   break;  }
    }
